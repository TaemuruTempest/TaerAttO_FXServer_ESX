...
function OpenActionMenuInteraction(target)
	...
	if isCarry then
		TriggerServerEvent('taeratto_barbie:server:ExitFromLyftup', GetPlayerServerId(closestPlayer))
		ClearPedTasksImmediately(PlayerPedId())
		isCarry = false
	else
		if distance ~= -1 and distance <= 3.0 then
			-- exports['Monster_base']:textNotification("คุณกำลังยกบุคคลนี้ขึ้น")
			TriggerServerEvent('taeratto_barbie:server:RequestLyftup', GetPlayerServerId(closestPlayer), IsPedDeadOrDying(GetPlayerPed(closestPlayer)))
...
RegisterNetEvent('taeratto_barbie:client:SendAcceptToSource')
AddEventHandler('taeratto_barbie:client:SendAcceptToSource', function()
	Citizen.Wait(200) --- ปรับดีเลย์อุ้ม
...
RegisterNetEvent('taeratto_barbie:client:RequestReceive')
AddEventHandler('taeratto_barbie:client:RequestReceive', function(dead)
	isRequestAnim = true
	if dead then
		local closestPlayer, distance = ESX.Game.GetClosestPlayer()
		TriggerServerEvent('taeratto_barbie:server:AcceptRequest', GetPlayerServerId(closestPlayer))
		isRequestAnim = false
	end
end)
...
RegisterNetEvent('taeratto_barbie:client:SendAcceptToTarget')
AddEventHandler('taeratto_barbie:client:SendAcceptToTarget', function(target)
...
RegisterNetEvent('taeratto_barbie:client:ReleaseTarget')
AddEventHandler('taeratto_barbie:client:ReleaseTarget', function()
	if isCarry then
